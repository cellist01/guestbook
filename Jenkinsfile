...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="cellist01/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', url:'https://github.com/cellist01/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/cellist01/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://cellist01:${GITHUB_ACCESS_TOKEN}@github.com/cellist01/guestbook-config.git"
                    '''
                }
            }
        }
